...
function assertSlot (template, outputSlot, options) {
  const fTemplate = template.replace(/[\s\r\n]*/g, '').replace(/:/g, ' :')
  const compiled = compile(fTemplate, {})
  const output = compileToWxml(compiled, options)
  const slotWxml = Object.values(output.slots).map(({code}) => code).join('')
  console.log('template -> ', fTemplate)
  console.log('output -> ', output.code)
  console.log('slotWxml -> ', slotWxml)
  console.log('slots -> ', output.slots)
